...
class Between extends BaseRule
{
    ...
    public function validate($input)
    {
        ...
        if ($isMinMaxSet && ($lowerLimitFailed === true || $upperLimitFailed === true)) {
            ...
            yield $this->createError($this->getParameterName(), $input, MessageCodes::BETWEEN, $context);
        } elseif ($lowerLimitFailed === true && $upperLimitFailed === false) {
            $context = [static::CONTEXT_MIN => $this->min, static::CONTEXT_MAX => $this->max];
            yield $this->createError($this->getParameterName(), $input, MessageCodes::BETWEEN_MIN, $context);
        } elseif ($lowerLimitFailed === false && $upperLimitFailed === true) {
            $context = [static::CONTEXT_MIN => $this->min, static::CONTEXT_MAX => $this->max];
            yield $this->createError($this->getParameterName(), $input, MessageCodes::BETWEEN_MAX, $context);
        }
    }
}
